	...
	: Scene(gameEngine)
{
	...
	// Register Action
	{
		...
		RegisterAction(sf::Keyboard::Space, "TOGGLE_SEEK_FLEE");
	}
	...
	// Load Level
	{
		...
		while (fin >> label)
		{
			if (label == "AICharacter")
			{
				...
				CTarget& targetToSeek = aiCharacter->AddComponent<CTarget>();
			...
			else if (label == "Text")
			{
				...
				auto& textComponent = m_algorithmDescription->AddComponent<CText>(text, m_game->assets().GetFont("FontTech"), 28, sf::Color{ 255, 255, 255 });
...
void SceneKinematic::SDoAction(const Action& action)
{
	if (action.Type() == "START")
	{
		...
		else if (action.Name() == "TOGGLE_SEEK_FLEE")
		{
			m_algorithmType = (KinematicAlgorithmType)((m_algorithmType + 1) % (KinematicAlgorithmType::Max));
			switch (m_algorithmType)
			{
				case KinematicAlgorithmType::Seek:
				{
...
void SceneKinematic::SSeek()
{
	...
	{
		if (!e->HasComponent<CTransform>()) continue;
		if (!e->HasComponent<CTarget>()) continue;

		CTransform& transform = e->GetComponent<CTransform>();
		const CTarget& target = e->GetComponent<CTarget>();
